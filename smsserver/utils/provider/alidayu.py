...
class ALiDaYuClient(BaseClient):
    ...
    支持类型
    可以发送国内短信、语音
    短信通知：同时用于验证码和通知类模板消息，发送需要提供模板 ID 和模板中的变量，支持发送多个
    语音通知：TTS 文字转语音（使用类似短信的那种模板），另外还可以播放固定的语音文件，支持发送多个
    推广短信：用于推广，模板中不能设置变量，使用和短信通知相同的接口

    API
    短信发送：单个或多个，短信通知或推广短信
    文本转语音：播放 TTS
    语音通知：播放固定语音文件的通知

    使用
    - 用短信发送 API 来发送验证码和通知类模板消息
    - 用文本转语音 API 发送语音验证码
    '''
